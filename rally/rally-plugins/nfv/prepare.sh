...
if [ -f nfv_test.cfg ]; then
  ...
  echo "nfv_test.cfg not found, fall back to enviroment varibles."
fi

function get_mac_from_slot() {
# arg1: pci slot number as in xx:xx:xx
  local slot=$1
  local mac=$(dmesg | sed -r -n "s/.*${slot}:.*([0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2})/\1/p" | tail -1)
  echo $mac
}
...
traffic_src_mac=$(get_mac_from_slot ${traffic_gen_src_slot})
traffic_dst_mac=$(get_mac_from_slot ${traffic_gen_dst_slot})
...
source $rcfile || error "can't load rcfile"
...
for i in $(seq 0 $num_vm); do
  neutron net-create provider-nfv$i --provider:network_type vlan --provider:physical_network dpdk$(($i % 2)) --provider:segmentation_id $((data_vlan_start + $i)) --port_security_enabled=False
  sleep 1
  neutron subnet-create --name provider-nfv$i --disable-dhcp --gateway 192.168.$i.254 provider-nfv$i 192.168.$i.0/24
done

tmpfile="/tmp/neutron_nets"
...
mac2=`get_vm_mac demo1${num_vm} provider-nfv${num_vm}`


if [[ ${run_pbench} == "yes" ]]; then
$PWD/start_pbench.sh
pbench-trafficgen --config="pbench-trafficgen" --num-flows=128 --traffic-directions=bidirec --src-ips=192.168.0.100,192.168.${num_vm}.100 --dst-ips=192.168.${num_vm}.100,192.168.0.100 --flow-mods=src-ip --traffic-generator=moongen-txrx --devices=${traffic_gen_src_slot},${traffic_gen_dst_slot} --vlan-ids=${data_vlan_start},$((data_vlan_start+num_vm)) --rate=${traffic_rate_init} --search-runtime=${search_runtime} --validation-runtime=${validation_runtime} --max-loss-pct=${traffic_loss_pct}
pbench-move-results
fi
